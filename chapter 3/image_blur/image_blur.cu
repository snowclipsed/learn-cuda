...
__global__ void blur_kernel(unsigned char* img_in, unsigned char* img_out, int width, int height, int channels){
    ...
    for(int ch = 0; ch<channels ; ch++){
        ...
        for(int blur_row = -BLUR_SIZE; blur_row <= BLUR_SIZE; blur_row++){
            ...
            img_out[(row * width + col) * channels + ch] = (unsigned char)(pixel_value/num_pixels);
        }        
    }
}
...
int width, height, channels;
    ...
    CHECK_CUDA_ERROR(cudaMemcpy(img_in_d, img_in_h, size, cudaMemcpyHostToDevice));

    dim3 blockDim(32, 32, 1);
    dim3 gridDim((width + blockDim.x - 1) / blockDim.x, (height + blockDim.y - 1) / blockDim.y, 1);

    blur_kernel<<<gridDim, blockDim>>>(img_in_d, img_out_d, width, height, channels);

    CHECK_CUDA_ERROR(cudaDeviceSynchronize());
    CHECK_CUDA_ERROR(cudaMemcpy(img_out_h, img_out_d, size, cudaMemcpyDeviceToHost));
